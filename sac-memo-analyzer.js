...
(function(){
  ...
  async function extractPdfText(file) {
    ...
    try { buffer = await file.arrayBuffer(); } catch (e) { return ''; }
    if (!buffer) return '';

    // المحاولة 1: مكتبة pdf.js الرسمية (الأدق للنصوص العربية)
    try {
      if (!window.pdfjsLib) {
        await new Promise((resolve) => {
          ...
          setTimeout(resolve, 12000); // مهلة أمان
        });
      }
    ...
    } catch (e) { /* ننتقل للاستخلاص الاحتياطي */ }

    // المحاولة 2 (احتياطية): مسح البايتات مباشرة عند فشل pdf.js
    if (!extractedText || extractedText.trim().length < 25) {
      try {
        const bytes = new Uint8Array(buffer);
        let tempStr = '';
        const limit = Math.min(bytes.length, 800000);
  ...
  /* =========================================================================
     ...
     ========================================================================= */
  window.__sacMemoTextAnalysis = function(cleanText, fileName, fileSize) {
  ...
  async function generateDynamicSacMemoReport(file) {
    ...
    try {
      ...
      const result = window.__sacMemoTextAnalysis(extractedText, file.name, file.size);
      ...
      reportContainer.innerHTML = `
        <div style="background:#fef2f2; border:2px solid #f87171; border-radius:16px; padding:18px 20px; text-align:center;">
          ...
          <div style="font-weight:800; color:#991b1b; font-size:1.05rem; margin:8px 0;">تعذّر قراءة محتوى ملف الـ PDF</div>
          <p style="font-size:0.9rem; color:#7f1d1d; line-height:1.7; margin:0 0 12px;">
            الملف قد يكون محمياً بكلمة سر، أو ممسوحاً ضوئياً (صور فقط دون طبقة نصية).<br>
            جرّب ملف PDF يحتوي نصاً قابلاً للتحديد والنسخ، أو أعد المحاولة باتصال أنترنت سليم.
          </p>
